      ...
      <div class="row">
        <div class="col-lg-9 order-lg-last col-md-12 order-md-first">
          ...
          <div v-else-if="showErrorMessage" class="error text-center mt-5">
            <h2>
              Произошла ошибка при загрузке товаров.<br />Попробуйте
        ...
        <ClientOnly>
          <FiltersAppCategoryFilters
            ...
            :patterns="store.filters.patterns"
            :weights="store.filters.weights"
            :colors="store.filters.colors"
            :glues="store.filters.glues"
            :mixture_types="store.filters.mixture_types"
            :seams="store.filters.seams"
            :textures="store.filters.textures"
            :countries="store.filters.countries"
            :sizes="store.filters.sizes"
            :materials="store.filters.materials"
            :waterproofs="store.filters.waterproofs"
            :collections="store.filters.collections"
            :volumes="store.filters.volumes"
            :product_weights="store.filters.product_weights"
            :installation_types="store.filters.installation_types"
            :shapes="store.filters.shapes"
            :applications="store.filters.applications"
            :drying_times="store.filters.drying_times"
            :package_weights="store.filters.package_weights"
            :min_temps="store.filters.min_temps"
            :max_temps="store.filters.max_temps"
            :consumptions="store.filters.consumptions"
...
useHead(
  computed(() => ({
    ...
    meta: [
      ...
      {
        name: 'keywords',
        content: categoryData.value?.name
