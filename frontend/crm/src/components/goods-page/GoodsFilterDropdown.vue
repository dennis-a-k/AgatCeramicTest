<template>
  <div class="relative" ref="filterRef" @click="closeFilter">
    ...
    <div v-show="showFilter" @click.stop
      ...
      <div class="mb-5">
        <FilterCheckbox id="checkboxLabelSale" label="Распродажа" :checked="checkboxSale" @update:checked="emit('update:checkboxSale', $event)" />
      </div>
      <div class="mb-5">
        <FilterCheckbox id="checkboxLabelNoSale" label="Не распродажа" :checked="checkboxNoSale" @update:checked="emit('update:checkboxNoSale', $event)" />
      </div>
      <div class="mb-5">
        <FilterCheckbox id="checkboxLabelPublished" label="Опубликован" :checked="checkboxPublished" @update:checked="emit('update:checkboxPublished', $event)" />
      </div>
      <div>
        <FilterCheckbox id="checkboxLabelNoPublished" label="Не опубликован" :checked="checkboxNoPublished" @update:checked="emit('update:checkboxNoPublished', $event)" />
      </div>
    </div>
  </div>
</template>
