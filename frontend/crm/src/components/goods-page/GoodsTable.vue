    ...
    <div class="space-y-5">
      <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="max-w-full overflow-x-auto custom-scrollbar">
          ...
          <div v-else-if="products.length === 0" class="flex flex-col justify-center items-center h-screen menu-item-icon-active text-center font-bold text-theme-xl m-5">
            Товары не найдены
          </div>
              ...
              <tr class="border-gray-100 border-y bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                <th class="cursor-pointer px-5 py-3 text-center w-1/11 sm:px-6" @click="handleSortBy('article')">
                  <div class="flex items-center gap-3">
                    ...
                    <span class="flex flex-col gap-0.5">
                      <svg :class="sort.key === 'article' && sort.asc ? 'text-gray-500 dark:text-gray-400' : 'text-gray-300 dark:text-gray-400/50'"
                        width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-gray-500 dark:text-gray-400">
                      ...
                      <svg :class="sort.key === 'article' && !sort.asc ? 'text-gray-500 dark:text-gray-400' : 'text-gray-300 dark:text-gray-400/50'"
                        width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-gray-300 dark:text-gray-400/50">
                ...
                <th class="cursor-pointer px-5 py-3 text-center w-2/11 sm:px-6" @click="handleSortBy('name')">
                  <div class="flex items-center gap-3">
                    ...
                    <span class="flex flex-col gap-0.5">
                      <svg :class="sort.key === 'name' && sort.asc ? 'text-gray-500 dark:text-gray-400' : 'text-gray-300 dark:text-gray-400/50'"
                        width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-gray-500 dark:text-gray-400">
                        <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="currentColor"></path>
                      </svg>
                      <svg :class="sort.key === 'name' && !sort.asc ? 'text-gray-500 dark:text-gray-400' : 'text-gray-300 dark:text-gray-400/50'"
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="product in products" :key="product.id" class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6">
                  ...
                  <p v-else-if="product.unit === 'кв.м'" class="text-gray-500 text-center text-theme-sm dark:text-gray-400">м²</p>
                  <p v-else class="text-gray-500 text-center text-theme-sm dark:text-gray-400">{{ product.unit }}</p>
                </td>
                <td class="px-5 py-4 sm:px-6 text-center">
                  <span v-if="product.is_published === true" class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-success-50 text-success-700 dark:bg-success-500/15 dark:text-success-500">Опубликован</span>
                  <span v-if="product.is_published === false" class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-gray-100 text-gray-700 dark:bg-white/5 dark:text-white/80">Скрыт</span>
                </td>
                <td class="px-5 py-4 sm:px-6 text-center">
                  <span v-if="product.is_sale === true" class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-error-50 text-error-700 dark:bg-error-500/15 dark:text-error-500">Распродажа</span>
                  <span v-if="product.is_sale === false" class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-gray-100 text-gray-700 dark:bg-white/5 dark:text-white/80">Нет</span>
                </td>
                <td class="px-5 py-4 sm:px-6 flex justify-center gap-3">
                  <div class="inline-flex items-center justify-center gap-2 rounded-lg transition shadow-theme-xs bg-white text-gray-700 ring-1 ring-gray-300 hover:bg-success-50 hover:ring-success-300 hover:text-success-700 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-success-500/15 dark:hover:ring-success-500/50 dark:hover:text-success-500 cursor-pointer p-1" @click="handleEdit(product)">
...
const confirmDelete = async () => {
  ...
  const success = await handleDelete(selectedProduct.value)
  if (success) {
    showAlert('success', 'Успешно', `Товар успешно удалён`)
    // Обновить список товаров
    emit('fetchProducts')
  } else {
    showAlert('error', 'Ошибка', 'Не удалось удалить товар')
  }
}
</script>
