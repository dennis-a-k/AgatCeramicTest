...
import { useProductCreator } from '@/composables/useProductCreator'
import { useProductValidation } from '@/composables/useProductValidation'

const imageUploadRef = ref()

const currentPageTitle = 'Создание нового товара'
...
const isCeramicCategory = computed(() => {
  ...
  const cat = categories.value.find((c: any) => c.value === Number(product.value.category_id))
  if (!cat) return false
