<template>
  <div class="relative sm:min-w-[200px] sm:w-auto" ref="multiSelectRef">
    <div @click="toggleDropdown"
      ...
      <span v-if="!selectedItem" class="text-gray-400"> Все статусы </span>
    ...
    <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <div v-if="isOpen" class="absolute z-10 w-full mt-1 bg-white rounded-lg shadow-sm dark:bg-gray-900">
        <ul class="overflow-y-auto divide-y divide-gray-200 custom-scrollbar max-h-60 dark:divide-gray-800"
          role="listbox">
          <li v-for="status in statuses" :key="status.id" @click="handleToggleItem(status)"
            ...
            :class="{ 'bg-gray-50 dark:bg-white/[0.03]': isSelected(status) }" role="option"
            :aria-selected="isSelected(status)">
            <span class="grow">{{ status.label }}</span>
            <svg v-if="isSelected(status)" class="w-5 h-5 text-gray-400 dark:text-gray-300" fill="none"
