<template>
  <div class="mb-6">
    ...
    <div class="h-full">
      <div
        class="rounded-2xl border border-warning-200 bg-white p-5 dark:border-warning-800 dark:bg-white/[0.03] md:p-6 h-full flex flex-col">
        ...
        <div class="flex items-end justify-between mt-5 flex-1">
          <div>
            <span class="font-bold text-sm text-gray-500 dark:text-warning-400">
              Новые заказы
            </span>
            <h4 class="mt-2 font-bold text-warning-600 text-title-sm dark:text-white/90">
              {{ statistics.current.pending }}</h4>
          </div>
          <span
    ...
    <div class="h-full">
      <div
        class="rounded-2xl border border-blue-light-200 bg-white p-5 dark:border-blue-light-800 dark:bg-white/[0.03] md:p-6 h-full flex flex-col">
        ...
        <div class="flex items-end justify-between mt-5 flex-1">
          <div>
            <span class="font-bold text-sm text-gray-500 dark:text-blue-light-400">
              На выполнении
            </span>
    ...
    <div class="h-full">
      <div
        class="rounded-2xl border border-success-200 bg-white p-5 dark:border-success-800 dark:bg-white/[0.03] md:p-6 h-full flex flex-col">
        ...
        <div class="flex items-end justify-between mt-5 flex-1">
          <div>
            <span class="font-bold text-sm text-gray-500 dark:text-success-400">
              Доставлено
            </span>
            <h4 class="mt-2 font-bold text-success-600 text-title-sm dark:text-white/90">
              {{ statistics.current.shipped }}</h4>
          </div>
          <span :class="[
            'flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-xs font-medium',
    ...
    <div class="h-full">
      <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 h-full flex flex-col">
        <div class="flex items-start justify-between">
        ...
        <div class="flex items-end justify-between mt-5 flex-1">
          <div>
            <span class="font-bold text-sm text-gray-500 dark:text-gray-400">
              Сумма выполненых заказов
            </span>
            <h4 class="mt-2 font-bold text-gray-800 text-xl dark:text-white/90">
              {{ new Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(statistics.current.total_amount) }}
            </h4>
          </div>
          <span :class="[
            'flex items-center gap-1 rounded-full py-0.5 pl-2 pr-2.5 text-xs font-medium',
