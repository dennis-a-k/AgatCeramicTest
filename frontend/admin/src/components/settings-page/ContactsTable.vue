  ...
  <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
    ...
    <div class="my-6">
      ...
      <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800">
        <span class="text-gray-500 text-theme-sm dark:text-gray-400">Телефон</span>
        <span class="text-right text-gray-500 text-theme-sm dark:text-gray-400">
          {{ contacts.phone || 'Не указано' }}
        </span>
      </div>
      <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800">
        <span class="text-gray-500 text-theme-sm dark:text-gray-400">Почта</span>
        <span class="text-right text-gray-500 text-theme-sm dark:text-gray-400">
          {{ contacts.email || 'Не указано' }}
        </span>
      </div>
      <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800">
        <span class="text-gray-500 text-theme-sm dark:text-gray-400">Telegram</span>
        <span class="text-right text-gray-500 text-theme-sm dark:text-gray-400">
          {{ contacts.telegram ? '@' + contacts.telegram : 'Не указано' }}
        </span>
      </div>
      <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-800">
        <span class="text-gray-500 text-theme-sm dark:text-gray-400">WhatsApp</span>
        <span class="text-right text-gray-500 text-theme-sm dark:text-gray-400">
          {{ contacts.whatsapp ? '+' + contacts.whatsapp : 'Не указано' }}
        </span>
      </div>
    </div>
  </div>

  <ContactsModal :is-visible="isModalVisible" :contacts="contacts" :errors="errors" @close="closeModal"
    @save="saveContacts" />
</template>
