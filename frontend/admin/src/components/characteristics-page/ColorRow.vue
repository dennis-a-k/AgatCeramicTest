  ...
  <tr class="border-t border-gray-100 dark:border-gray-800">
    ...
    <td class="py-3 whitespace-nowrap flex justify-center items-center">
      <div class="w-8 h-8 rounded-full border border-gray-300 dark:border-gray-600"
        :style="{ backgroundColor: color.hex }" :aria-label="`Цвет ${color.name} с кодом ${color.hex}`"></div>
    </td>
    <td class="py-3 whitespace-nowrap text-center">
      <button
        ...
        @click="openEditModal(color)" :aria-label="`Редактировать цвет ${color.name}`">
        <EditIcon />
      </button>
      <button
        ...
        @click="deleteColor(color.id)" :aria-label="`Удалить цвет ${color.name}`">
        <DeleteIcon />
      </button>
    </td>
  </tr>
