      ...
      <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
        <div v-for="(attr, index) in product.attribute_values" :key="attr.attribute.id">
          <div v-if="attr.attribute.type === 'string'">
            <label :for="`attr-string${attr.attribute.id}`"
          ...
          <div v-else-if="attr.attribute.type === 'number'">
            <label :for="`attr-number${attr.attribute.id}`"
              class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ attr.attribute.name }} {{ attr.attribute.unit ? `(${attr.attribute.unit})` : '' }}
            </label>
            <input :id="`attr-number${attr.attribute.id}`" type="number" step="0.01" min="0"
              v-model.number="attr.number_value" :class="inputClass">
          </div>
          <div v-else-if="attr.attribute.type === 'boolean'" class="h-full flex">
            <Checkbox :id="'attr-' + attr.attribute.id" :label="attr.attribute.name"
              v-model:checked="attr.boolean_value" />
          </div>
          <div v-else-if="attr.attribute.type === 'text'">
            <label :for="`attr-text${attr.attribute.id}`"
              class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ attr.attribute.name }} {{ attr.attribute.unit ? `(${attr.attribute.unit})` : '' }}
            </label>
            <textarea :id="`attr-text${attr.attribute.id}`" rows="4" v-model="attr.text_value"
